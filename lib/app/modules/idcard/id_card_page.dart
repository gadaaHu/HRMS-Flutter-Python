...
class IDCARDPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Safely extract navigation arguments
    final IDCARDArguments? args = Get.arguments is IDCARDArguments
        ? Get.arguments as IDCARDArguments
        : null;

    // Schedule setIDCard after build to avoid setState-during-build errors
    if (args != null) {
      SchedulerBinding.instance.addPostFrameCallback((_) {
        try {
          final controller = Get.find<IDCARDController>();
                                  ...
                                  Text(
                                      AppUtils.getStandardFormat(
                                        AppUtils.getReversedDate(
                                          controller.idCardDetails.value?.dob.isNotEmpty == true
                                              ? controller.idCardDetails.value!.dob
                                              : "01/01/2000",
                                        ),
                                      ),
                                      style: TextStyle(
